from tkinter import Toplevel, Label, NSEW, Entry
...
class EditCompletedTreatment(Toplevel):
    ...
    def init_widgets(self):
        ...
        # Header label
        main_label = Label(
            self, text="Промяна на завършена процедура", bg=self.root.BG_COLOR
        )
        main_label.grid(row=0, column=0, columnspan=4, sticky=NSEW)

        # Info labels for customer, zone and date
        date_label = Label(
            self, text="Дата: " + self.root.get_date_string(), bg=self.root.BG_COLOR
        )
        ...
        zone_label = Label(
            self, text="Зона: " + self.treatment_data[1], bg=self.root.BG_COLOR
        )
        zone_label.grid(row=3, column=0, sticky=NSEW)

        # Impulse widgets
        i_label = Label(
            self, text="Импулси: " + str(self.treatment_data[2]), bg=self.root.BG_COLOR
        )
        ...
        i_button = ctk.CTkButton(
            self,
            text="Промени импулси",
        ...
        # Frequency widgets
        hz_label = Label(
            self, text="Честота: " + str(self.treatment_data[3]), bg=self.root.BG_COLOR
        )
        ...
        hz_button = ctk.CTkButton(
            self,
            text="Промени честота",
        ...
        # Energy widgets
        j_label = Label(
            self, text="Джаули: " + str(self.treatment_data[4]), bg=self.root.BG_COLOR
        )
