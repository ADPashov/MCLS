from tkinter import Toplevel, Label, NSEW, StringVar, OptionMenu, Grid
...
class EditTreatment(Toplevel):
    ...
    def init_widgets(self):
        ...
        main_label = Label(self,
                           text="Промяна на процедура " + self.treatment_data[
                               1] + ' на' + self.customer_data + ' от ' + self.time + ' на ' + self.root.get_date_string(),
        ...
        # Duration widgets
        duration_label = Label(self, text='Нова продължителност: ', bg=self.root.BG_COLOR)
        duration_label.grid(row=1, column=5, sticky=NSEW)

        duration_string = StringVar(self)
        duration_string.set(list(self.root.DURATIONS.keys())[self.treatment_data[2] - 1])
        ...
        edit_button = ctk.CTkButton(self, text='Промени Процедура',
                                    command=lambda: self.root.edit_treatments([[self.treatment_data[0]],
                                                                               [self.treatment_data[1]],
                                                                               [day_string.get(),
                                                                                [k for k, v in self.root.MONTHS.items()
                                                                                 if v == month_string.get()][0],
                                                                                year_string.get()], time_string.get(),
                                                                               self.root.DURATIONS[
                                                                                   duration_string.get()]]))
