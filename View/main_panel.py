...
class MainPanel(Frame):
    ...
    def init_find(self):
        ...
        self.find_widgets['find_label'] = Label(self, text="Търсене на клиент", bg=self.root.BG_COLOR)
        ...
        # Find button
        self.find_widgets['find_button'] = ctk.CTkButton(self, text="Намиране на клиент",
        ...
        self.find_widgets['add_button'] = ctk.CTkButton(self, text='Добавяне на клиент',
    ...
    def get_search_criteria(self):
        ...
        """
        return [self.find_widgets['id_box'].get(),
    ...
    def init_customer(self):
        ...
        # Section Header Label
        self.customer_widgets['text_customer'] = Label(self, text="Клиент", bg=self.root.BG_COLOR)
        self.customer_widgets['text_customer'].grid(row=0, column=2, columnspan=2, sticky=NSEW)

        # ID labels
        self.customer_widgets['id_label'] = Label(self, text='Клиентски номер', bg=self.root.BG_COLOR)
        ...
        # Name labels
        self.customer_widgets['name_label'] = Label(self, text="Име", bg=self.root.BG_COLOR)
        self.customer_widgets['name_label'].grid(row=2, column=2, sticky=NSEW)

        self.customer_widgets['name_result'] = Label(self, text='', bg=self.root.BG_COLOR)
        self.customer_widgets['name_result'].grid(row=2, column=3, sticky=NSEW)

        # Skin type labels
        self.customer_widgets['skintype_label'] = Label(self, text="Фототип Кожа", bg=self.root.BG_COLOR)
        self.customer_widgets['skintype_label'].grid(row=3, column=2, sticky=NSEW)

        self.customer_widgets['skintype_result'] = Label(self, text='', bg=self.root.BG_COLOR)
        self.customer_widgets['skintype_result'].grid(row=3, column=3, sticky=NSEW)

        # Phone number labels
        self.customer_widgets['phone_label'] = Label(self, text="Телефон", bg=self.root.BG_COLOR)
        self.customer_widgets['phone_label'].grid(row=4, column=2, sticky=NSEW)

        self.customer_widgets['phone_result'] = Label(self, text='', bg=self.root.BG_COLOR)
        self.customer_widgets['phone_result'].grid(row=4, column=3, sticky=NSEW)

        # Mail labels
        self.customer_widgets['mail_label'] = Label(self, text="Имейл", bg=self.root.BG_COLOR)
        self.customer_widgets['mail_label'].grid(row=5, column=2, sticky=NSEW)

        self.customer_widgets['mail_result'] = Label(self, text='', bg=self.root.BG_COLOR)
        self.customer_widgets['mail_result'].grid(row=5, column=3, sticky=NSEW)

        # Notes label and text box
        self.customer_widgets['notes_label'] = Label(self, text="Бележки на клиента", bg=self.root.BG_COLOR)
        self.customer_widgets['notes_label'].grid(row=6, column=2, columnspan=2, sticky=NSEW)

        self.customer_widgets['notes_box'] = Listbox(self, bg=self.root.BOX_COLOR, width=50, height=1,
                                                     highlightbackground=self.root.BG_COLOR)
        self.customer_widgets['notes_box'].grid(row=7, column=2, columnspan=2, sticky=NSEW)
        ...
        self.customer_widgets['add_note'] = ctk.CTkButton(self, text='Добавяне на бележка',
    ...
    def init_calendar(self):
        ...
        self.calendar_widgets['treatment_times'] = self.root.get_treatment_times().copy()
        # Calendar label
        self.calendar_widgets['text'] = Label(self, text='Календар', bg=self.root.BG_COLOR)
        ...
        self.calendar_widgets['calendar'].configure(normalbackground=self.root.CALENDAR_NORMALBG)
        self.calendar_widgets['calendar'].configure(weekendbackground=self.root.CALENDAR_WEEKENDSBG)
        self.calendar_widgets['calendar'].configure(othermonthbackground=self.root.CALENDAR_OTHERMONTHBG)
        self.calendar_widgets['calendar'].configure(othermonthwebackground=self.root.CALENDAR_OTHERMONTWEHBG)
    ...
    def show_today(self, today):
        ...
        self.calendar_widgets['calendar'].selection_set(datetime.date(today[2], today[1], today[0]))
    ...
    def flush_find(self):
        self.find_widgets['results_box'].delete(0, self.find_widgets['results_box'].size())
    ...
    def flush_select_customer(self):
        ...
        self.customer_widgets['notes_box'].delete(0, self.customer_widgets['notes_box'].size())
        self.flush_customer_treatments()

    def flush_customer_treatments(self):
        self.customer_widgets['treatments_box'].delete(0, self.customer_widgets['treatments_box'].size())
